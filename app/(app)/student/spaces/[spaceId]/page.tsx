...
function isFsTimestampLike(x: unknown): x is { toMillis: () => number } {
  return !!x && typeof x === "object" && "toMillis" in x && typeof (x as { toMillis?: unknown }).toMillis === "function";
}
...
function toMillisAny(x: unknown): number {
  if (isFsTimestampLike(x)) {
    try {
      ...
      void e; // satisfy no-empty & keep robust
    }
  }
...
function normalizeStatus(s: unknown): "submitted" | "needs_work" | "approved" | "draft" | "unknown" {
  const v = String(s ?? "").toLowerCase().trim();
  if (v === "needs_work" || v === "needswork" || v === "needs-work") return "needs_work";
  // ✅ treat reviewed as approved in UI
  if (v === "reviewed" || v === "approved" || v === "ok" || v === "passed" || v === "done") return "approved";
  if (v === "submitted" || v === "sent" || v === "delivered") return "submitted";
  if (v === "draft") return "draft";
  return "unknown";
}

function statusLabel(st: ReturnType<typeof normalizeStatus>) {
  switch (st) {
    case "submitted":
      return { text: "Sendt inn", hint: "Venter på tilbakemelding" };
    case "needs_work":
      return { text: "Må forbedres", hint: "Lærer har åpnet for forbedring" };
    case "approved":
      return { text: "Godkjent", hint: "Ferdig" };
    case "draft":
      return { text: "Kladd", hint: "Ikke sendt inn" };
    default:
      return { text: "Ukjent", hint: "" };
  }
}

function pillStyle(st: ReturnType<typeof normalizeStatus>) {
  if (st === "needs_work") return { border: "1px solid rgba(245,158,11,0.60)", background: "rgba(245,158,11,0.18)" };
  if (st === "approved") return { border: "1px solid rgba(46,204,113,0.60)", background: "rgba(46,204,113,0.16)" };
  return { border: "1px solid rgba(0,0,0,0.22)", background: "rgba(0,0,0,0.04)" };
}

function StatusPill({ status }: { status: ReturnType<typeof normalizeStatus> }) {
  const ui = statusLabel(status);
  return (
    <span
      title={ui.hint}
      style={{
...
function detectTeacherMessage(data: Record<string, unknown>) {
  ...
  if (tf && isRecord(tf)) {
    ...
    // ✅ also count “message exists” if teacherFeedback.updatedAt is set
...
export default function StudentSpaceDetailPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      unsub = onSnapshot(
        ...
        (e: unknown) => setErr(errMessage(e, "Kunne ikke lese klasse"))
      );
    } catch (e: unknown) {
    ...
    try {
      ...
      unsub = onSnapshot(
        ...
        (snap) => {
          ...
          snap.forEach((d: QueryDocumentSnapshot<DocumentData>) => out.push({ id: d.id, data: d.data() as AssignmentDoc }));
          setAssignments(out);
        },
        (e: unknown) => setAssignErr(errMessage(e, "Kunne ikke lese oppgaver"))
      );
    } catch (e: unknown) {
      setAssignErr(errMessage(e, "Kunne ikke starte lytting på oppgaver"));
    }
    ...
    try {
      ...
      unsub = onSnapshot(
        ...
        (e: unknown) => {
          ...
          setSubsErr(`${code}: ${errMessage(e, "Kunne ikke hente innleveringer")}`);
          setSubsLoading(false);
        }
      );
    } catch (e: unknown) {
      const code =
        e && typeof e === "object" && "code" in e ? String((e as { code?: unknown }).code ?? "error") : "error";
      setSubsErr(`${code}: ${errMessage(e, "Kunne ikke starte lytting")}`);
      setSubsLoading(false);
    }
  ...
  // Aktiv oppgave-kortet vises kun hvis det finnes aktiv oppgave OG eleven ikke har levert (ingen row i spaceSubmissions ennå).
  ...
  const activeAssignmentHref = activeAssignmentId ? `/student/spaces/${spaceId}/assignments/${activeAssignmentId}` : null;
  ...
  async function moveToArchive(subId: string) {
    ...
    try {
      ...
      setArchiveMsg("Flyttet til arkiv ✅");
    } catch (e: unknown) {
      const code =
        e && typeof e === "object" && "code" in e ? String((e as { code?: unknown }).code ?? "error") : "error";
      setArchiveMsg(`${code}: ${errMessage(e, "Kunne ikke flytte til arkiv")}`);
    } finally {
      setArchivingId(null);
  ...
  async function restoreFromArchive(subId: string) {
    ...
    try {
      ...
      setArchiveMsg("Flyttet tilbake ✅");
    } catch (e: unknown) {
      const code =
        e && typeof e === "object" && "code" in e ? String((e as { code?: unknown }).code ?? "error") : "error";
      setArchiveMsg(`${code}: ${errMessage(e, "Kunne ikke flytte tilbake")}`);
    } finally {
      setArchivingId(null);
      setTimeout(() => setArchiveMsg(null), 2500);
    }
  ...
  if (missing) {
    return (
      <div style={{ padding: 16 }}>
        <h1>Fant ikke klasse</h1>
        <div style={{ opacity: 0.75 }}>Denne klassen finnes ikke, eller du har ikke tilgang.</div>
        <div style={{ marginTop: 12 }}>
          <Link href="/student/spaces">← Tilbake til Mine klasser</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: 16, maxWidth: 900 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ opacity: 0.7 }}>
            Klassekode: <b>{spaceCode ?? "—"}</b>
          </div>
        </div>

        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <Link href="/student/spaces">← Mine klasser</Link>
        </div>
      </div>
      ...
      {/* 1) ACTIVE ASSIGNMENT */}
      <div style={{ marginTop: 14, border: "1px solid rgba(0,0,0,0.12)", borderRadius: 12, padding: 12, background: "white" }}>
        <div style={{ fontWeight: 900, marginBottom: 6 }}>Aktiv oppgave</div>
        ...
        {activeAssignmentId && shouldShowActiveAssignment && activeAssignmentHref ? (
          <>
            <div style={{ opacity: 0.92, marginBottom: 6, fontWeight: 900, fontSize: 16 }}>
              {activeLessonTitleFromSpace ?? safeString(activeAssignmentDoc?.title) ?? titleFor(activeAssignmentId)}
            </div>

            {safeString(activeAssignmentDoc?.level) || safeString(activeAssignmentDoc?.language) ? (
              <div style={{ fontSize: 12, opacity: 0.75, marginBottom: 10 }}>
                {[safeString(activeAssignmentDoc?.level), safeString(activeAssignmentDoc?.language)].filter(Boolean).join(" • ")}
              </div>
            ) : (
            ...
            <div style={{ marginTop: 10, opacity: 0.7 }}>Oppgaven ligger her når læreren har satt den som aktiv.</div>
          </>
        ) : (
          <div style={{ opacity: 0.75 }}>
            Ingen aktiv oppgave akkurat nå. Oppgaver du allerede har levert finner du under “Mine innleveringer”.
          </div>
        )}
      </div>

      {/* 2) MINE INNLEVERINGER */}
      <div style={{ marginTop: 14, border: "1px solid rgba(0,0,0,0.12)", borderRadius: 12, padding: 12, background: "white" }}>
        ...
        {!uid ? (
          <div style={{ marginTop: 12, opacity: 0.75 }}>Logg inn for å se innleveringer.</div>
        ) : subsLoading ? (
          <div style={{ marginTop: 12, opacity: 0.7 }}>Laster…</div>
        ) : subsErr ? (
          <div style={{ marginTop: 12, color: "crimson", whiteSpace: "pre-wrap" }}>{subsErr}</div>
        ) : activeSubs.length === 0 ? (
          <div style={{ marginTop: 12, opacity: 0.75 }}>Ingen innleveringer her ennå.</div>
        ) : (
          <div style={{ marginTop: 12, display: "grid", gap: 10 }}>
            {activeSubs.map((g) => {
              ...
              return (
                <div key={g.assignmentId} style={{ border: "1px solid rgba(0,0,0,0.12)", borderRadius: 12, padding: 12, background: "white" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                    <div style={{ minWidth: 240 }}>
                      <div style={{ fontWeight: 900, fontSize: 16, lineHeight: 1.25 }}>{r.title ?? titleFor(g.assignmentId)}</div>

                      {metaForRow(r) ? <div style={{ fontSize: 12, opacity: 0.75, marginTop: 6 }}>{metaForRow(r)}</div> : null}

                      <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                        <StatusPill status={r.status} />
                        {dateStr ? <span style={{ fontSize: 12, opacity: 0.75 }}>{dateStr}</span> : null}
                        {r.hasTeacherMessage ? <span style={{ fontSize: 12, opacity: 0.9 }}>✉️ Lærer har sendt deg en melding</span> : null}
                      </div>

                      {r.status === "needs_work" ? <div style={{ marginTop: 8, fontSize: 13, opacity: 0.9 }}>✍️ Åpnet for forbedring</div> : null}
                    </div>

                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                      {/* ✅ Flytt til arkiv: kun når approved (reviewed/approved) */}
                      {r.status === "approved" ? (
                        <button
                          type="button"
                          ...
                          title="Dette flytter innleveringen til ditt arkiv"
                        >
                          {archivingId === r.id ? "Flytter…" : "Flytt til arkiv"}
                        </button>
                      ) : null}
      ...
      {/* 3) ARKIV (manual) */}
      <div style={{ marginTop: 14, border: "1px solid rgba(0,0,0,0.12)", borderRadius: 12, padding: 12, background: "white" }}>
        <div style={{ fontWeight: 900, marginBottom: 6 }}>Arkiv</div>

        {!uid ? (
          <div style={{ opacity: 0.75 }}>Logg inn for å se arkiv.</div>
        ) : subsLoading ? (
          <div style={{ opacity: 0.7 }}>Laster…</div>
        ) : archivedSubs.length === 0 ? (
          <div style={{ opacity: 0.75 }}>Ingen arkiverte innleveringer enda.</div>
        ) : (
          <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
            {archivedSubs.map((g) => {
              ...
              return (
                <div key={g.assignmentId} style={{ border: "1px solid rgba(0,0,0,0.10)", borderRadius: 12, padding: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                    <div style={{ minWidth: 240 }}>
                      ...
                      <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                        <StatusPill status={r.status} />
                        {dateStr ? <span style={{ fontSize: 12, opacity: 0.75 }}>{dateStr}</span> : null}
                        {r.hasTeacherMessage ? <span style={{ fontSize: 12, opacity: 0.9 }}>✉️ melding</span> : null}
                      </div>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                      <button
                        ...
                        title="Flytt tilbake til Mine innleveringer"
                      >
                        {archivingId === r.id ? "Flytter…" : "Flytt tilbake"}
                      </button>
      ...
      <div style={{ marginTop: 14, border: "1px solid rgba(0,0,0,0.12)", borderRadius: 12, padding: 12, background: "white" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
          <div style={{ fontWeight: 900 }}>Alle oppgaver i klassen</div>
        ...
        {assignErr ? (
          ...
          <div style={{ marginTop: 10, opacity: 0.75 }}>Ingen oppgaver her ennå.</div>
        ) : (
          <div style={{ marginTop: 12, display: "grid", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
                    <div style={{ minWidth: 240 }}>
                      <div style={{ fontWeight: 900 }}>
                        {title} {archived ? <span style={{ fontWeight: 600, opacity: 0.6 }}>(arkivert)</span> : null}
                      </div>

                      {mine ? (
                        <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                          <span style={{ fontSize: 12, opacity: 0.65 }}>Din status:</span>
                          <StatusPill status={mine.status} />
                          {mine.hasTeacherMessage ? <span style={{ fontSize: 12, opacity: 0.9 }}>✉️ melding</span> : null}
                          {fmtDate(mine.updatedAtMs || mine.createdAtMs) ? (
                            <span style={{ fontSize: 12, opacity: 0.75 }}>{fmtDate(mine.updatedAtMs || mine.createdAtMs)}</span>
                          ) : null}
                        </div>
                      ) : (
                        <div style={{ marginTop: 8, fontSize: 12, opacity: 0.6 }}>Du har ikke levert inn denne ennå.</div>
                      )}
                    </div>
      ...
      <div style={{ marginTop: 14, opacity: 0.7 }}>
        Direkte lenke-format: <code>/student/spaces/{spaceId}/assignments{"{/{assignmentId}}"}</code>
      </div>
    </div>
  );
}