...
export default function LoginPage() {
  ...
  // ✅ Vi sjekker om brukeren er anonym, så vi kan LINK i stedet for sign-in/up
  ...
  async function handleEmail() {
    ...
    try {
      if (mode === "signin") {
        // Ved sign-in: vi bruker eksisterende signInWithEmail (også for anon)
        // (Linking for sign-in er mer komplisert hvis epost allerede finnes.)
        await signInWithEmail(e, password);
      } else {
        // Ved signup: hvis anon -> link (beholder uid), ellers normal signup
        if (isAnon) {
          await linkAnonymousWithEmailPassword(e, password);
          // displayName settes typisk separat (valgfritt). Du kan gjøre det senere.
        } else {
