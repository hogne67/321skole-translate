...
export default function PollDisplayPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!res.ok) {
      setError(isRecord(data) && typeof data.error === "string" ? data.error : "Kunne ikke hente avstemmingen.");
      return;
    }
    ...
    <main className="h-screen overflow-hidden bg-[#100a22] px-8 py-6 text-white">
      <div className="mx-auto flex h-full max-w-[1800px] flex-col">
        <header className="relative flex items-start justify-center gap-6 text-center">
          ...
          <div>
            <div className="text-sm font-black uppercase tracking-[0.22em] text-violet-300">321school avstemming</div>
            <h1 className="mt-3 max-w-5xl text-4xl font-black leading-tight">{session?.question ?? "Avstemming"}</h1>
            <p className="mt-2 text-lg font-bold text-white/60">{total} stemmer</p>
          </div>
          {hasTimer ? (
          ...
          <div className="relative min-h-0 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.04] p-8">
            ...
            {options.length ? (
                ...
                {options.map((item, index) => {
                  ...
                  return (
                    <div key={item.option} className="rounded-[1.5rem] bg-white/10 p-5">
                      ...
                      <div className="mt-2 text-lg font-bold text-white/60">{item.count} stemmer</div>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="flex h-full items-center justify-center text-center">
                <div>
                  <div className="text-5xl font-black">Venter på stemmer...</div>
