...
import { activateSchoolTeacher, getSchoolMember } from "@/lib/schools/server";
...
export async function POST(req: Request) {
  try {
    ...
    if (!targetUid) return json({ ok: false, error: "Missing targetUid" }, 400);
    ...
    const result = await activateSchoolTeacher({
      schoolId,
      targetUid,
      performedByUid: uid,
    });
    ...
    return json({ ok: false, error: message || "Failed to activate school teacher" }, 500);
  }
}
