...
import { disableSchoolTeacher, getSchoolMember } from "@/lib/schools/server";
...
export async function POST(req: Request) {
  try {
    ...
    if (!targetUid) {
      return json({ ok: false, error: "Missing targetUid" }, 400);
    }
    ...
    const result = await disableSchoolTeacher({
      schoolId,
      targetUid,
      performedByUid: uid,
    });
    ...
    return json({ ok: false, error: message || "Failed to disable school teacher" }, 500);
  }
}
