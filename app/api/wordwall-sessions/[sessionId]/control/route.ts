...
import { isRecord, safeString } from "@/lib/wordwallSessions";
...
export async function POST(req: Request, ctx: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    const [sessionSnap, userSnap] = await Promise.all([
      db.collection("wordwallSessions").doc(sessionId).get(),
    ...
    if (session.status === "finished") return json({ error: "Ordskyen er avsluttet." }, 409);
    ...
    const message = error instanceof Error ? error.message : "Could not control wordwall";
