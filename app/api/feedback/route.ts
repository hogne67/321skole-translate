...
function buildReadingLevelGuidance(level: string): string {
  ...
  if (normalized === "B2" || normalized === "C1" || normalized === "C2") {
    return [
      ...
      "Ikke foreslå å gå videre til B2 eller et lavere nivå når teksten allerede er på dette nivået eller høyere.",
      "Ved svært godt resultat kan neste steg være en lengre tekst, et mer krevende tema eller en mer nyansert tekst på samme nivå.",
    ].join(" ");
  }
  ...
  if (nextLevel[normalized]) {
    return [
      `Tekstens CEFR-nivå er ${normalized}.`,
      `Bare ved svært godt resultat og god forståelse kan du forsiktig foreslå å prøve en litt mer krevende tekst, for eksempel ${nextLevel[normalized]}.`,
      "Ellers bør neste steg være mer øving på samme nivå.",
    ].join(" ");
  }
