...
function copyFor(locale: string, role: GateRole) {
  const roleText =
    role === "teacher"
      ? {
        nb: "For å publisere og bruke KI-verktøy må e-posten være bekreftet.",
        en: "Publishing and AI tools require a verified email.",
        pt: "Publicar e usar ferramentas de IA exige e-mail confirmado.",
      }
      : role === "student"
        ? {
          nb: "Du kan lese og se innhold mens du venter, men du må bekrefte e-posten før du lager eget innhold eller bruker KI-verktøy.",
          en: "You can read and view content while waiting, but creating your own content or using AI tools requires a verified email.",
          pt: "Você pode ler e ver conteúdo enquanto espera, mas criar conteúdo próprio ou usar ferramentas de IA exige e-mail confirmado.",
        }
        : {
          nb: "For å lage familierom og bruke KI-verktøy må e-posten være bekreftet.",
          en: "Creating family rooms and using AI tools require a verified email.",
          pt: "Criar espaços familiares e usar ferramentas de IA exige e-mail confirmado.",
        };
  ...
  if (locale === "en") {
    return {
      ...
      text: roleText.en,
      expiredText: "The verification period has ended. You can still read and view content, but creating, publishing and AI tools require a verified email.",
  ...
  if (locale === "pt") {
    return {
      ...
      text: roleText.pt,
      expiredText: "O prazo de confirmação terminou. Você ainda pode ler e ver conteúdo, mas criar, publicar e usar IA exige e-mail confirmado.",
  ...
  return {
    ...
    text: `Du kan lese, se og studere mens du venter. ${roleText.nb}`,
    expiredText: "Bekreftelsesfristen er ute. Du kan fortsatt lese og se innhold, men for å lage, publisere og bruke KI må e-posten være bekreftet.",
...
export default function EmailVerificationGate({
  ...
  async function resend() {
    ...
    try {
      ...
      console.warn("email verification resend failed", err);
      const code = err && typeof err === "object" && "code" in err ? String(err.code) : "";
      setError(code.includes("too-many-requests") ? t.tooMany : t.failed);
    } finally {
      setBusy(false);
    }
  }
    ...
    try {
      ...
      const response = await fetch("/api/support-tickets", {
        ...
        body: JSON.stringify({
          category: "login",
          locale,
          page: typeof window !== "undefined" ? window.location.pathname : `/${role}`,
          name: auth.currentUser.displayName || "",
          contact: auth.currentUser.email || "",
          message:
            locale === "en"
              ? `${role} email verification: I have requested a verification email, but I have not received it and I have checked spam/junk.`
              : locale === "pt"
                ? `${role} confirmação de e-mail: solicitei o e-mail de confirmação, mas não recebi e já verifiquei spam/lixo eletrônico.`
                : `${role} e-postverifisering: Jeg har bedt om bekreftelsesmail, men har ikke mottatt den og har sjekket søppelpost/junk.`,
        }),
      });
